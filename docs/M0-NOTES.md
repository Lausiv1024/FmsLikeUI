...
- **書き込み後は電源ボタンを押す**。`flash` が `Hard resetting via RTS pin` と言っても、
  **ボードはダウンロードモードから自動で抜けません**。そのままシリアルを読むと、動いていない
  ボードを読むことになり、**0 バイト**が返ります。これは「ファームが黙っている」ようにしか見えず、
  クラッシュの切り分け中だと最悪の紛らわしさになります。ログが空なら、まず電源ボタンを疑うこと。
- **コンソールの出力先**: 既定の `CONFIG_ESP_CONSOLE_UART_DEFAULT` は UART0 に出すので、
  COM7(USB-Serial-JTAG)には何も来ません。実機のログを読むなら
  `CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG=y` にすること。パニックメッセージもこれ経由で出ます。
